...
def select_soluciones(conn, idSolucion=None):
    if idSolucion is None:
        sql = """select * from Soluciones"""
    else:
        sql = f"select * from Soluciones where solucionId = {idSolucion}"
    cur = conn.cursor()
    cur.execute(sql)
    filas = cur.fetchall()
    return filas

def select_solucionesXResolucion(conn,idResolucion):
    pass
