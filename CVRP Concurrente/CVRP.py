...
def getPermitidosMP(Aristas, A_solucion, umbralMin, umbralMax, solucion, ind_permitidos):
    ...
    for EP in Aristas:
        ...
        if aristaInv is not None:
            pertS = True
            del dictA[hInverso]
        if(not pertS and umbralMin <= EP.getPeso() and EP.getPeso() <= umbralMax):
...
class CVRP:
    def __init__(self, M, D, nroV, capac, archivo, carpeta, solI, tADD, tDROP, tiempo, porcentaje, optimo, coord = None, idInstancia = None):
        print("Ok1!!")
        self.getPermitidosMP = getPermitidosMP
        ...
        print("\nTiempo Sol Inicial: ", time()-tiempoIni)
    ...
    def tabuSearch(self):
        ...
        while(tiempoEjecuc < tiempoMax and porcentaje*100 > self.__porcentajeParada):
            if(cond_Optimiz):
                ...
                print("Tiempo get Permitidos: "+str(time()-tIni))
                self.__umbralMin = 0
    ...
    def getPermitidos(self, Aristas, umbralMax, solucion_refer):
        #print("Iniciamos")
        #tiempoIni = time()
        ...
        num_processes = 5
        manager = mp.Manager()
        self.retListIndPerm = manager.list()
        lenXProc = int(len(Aristas)/num_processes)
        processes = []
        A_solucion = solucion_refer.getA()
        for i in range(num_processes):
            if i == num_processes-1:
                AristasAux = Aristas[lenXProc*i:]
            else:
                AristasAux = Aristas[lenXProc*i:lenXProc*(i+1)]
            p = mp.Process(target=self.getPermitidosMP, args=(AristasAux, A_solucion, self.__umbralMin, umbralMax, solucion_refer, self.retListIndPerm))
            p.start()
            processes.append(p)
        for p in processes:
            p.join()

        # tiempoEjec = time() - tiempoIni
        # print("Tiempo de ejecuion usando PM (3 hilos): ", tiempoEjec)
        ...
        # tiempoEjec = time() - tiempoIni
        # print("Tiempo de ejecuion usando PM (3 hilos): ", tiempoEjec)
        # print("\nLen IndPerm: ", len(self.retListIndPerm))
        # print("Len Aristas: ", len(Aristas))
        # a = 1/0

        return self.retListIndPerm
