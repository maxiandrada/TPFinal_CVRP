...
def cargarRutasPool(rutas, capacidad, matrizDistancias, demanda, gr, R):
    #R = []
    for r in rutas:
        #S = Solucion(matrizDistancias, demanda, 0)
        S = Solucion(matrizDistancias, demanda, capacidad, G=gr, vacio=True)
...
class Solucion(Grafo):
    def __init__(self, M, Demanda, capacidad,G=None,vacio=False):
        ...
        self.cargarRutasPool = cargarRutasPool
        ...
        while(not sol_factible):
            ...
            if(strSolInicial==0):
                ...
                if (R == []):
                    ...
                    #rutas = self.cargarRutasMP(R, capacidad,G)
    ...
    def cargarRutasMP(self, rutas, capacidad, G):
        print("Iniciamos cargar Rutas")
        tiempoIni = time()
        num_processes = 1
        manager = mp.Manager()
        self.retListIndPerm = manager.list()
        lenXProc = int(len(rutas)/num_processes)
        processes = []

        for i in range(num_processes):
            if i == num_processes-1:
                rutasAux = rutas[lenXProc*i:]
            else:
                rutasAux = rutas[lenXProc*i:lenXProc*(i+1)]
            p = mp.Process(target=self.cargarRutasPool, args=(rutasAux, capacidad, self._matrizDistancias, self._demanda, G, self.retListIndPerm))
            p.start()
            processes.append(p)
        for p in processes:
            p.join()

        rutas = self.retListIndPerm
        tiempoEjec = time() - tiempoIni
        #print("rutas: "+str(rutas))
        print("Tiempo de ejecuion usando PM (2 hilos): ", tiempoEjec)    
        #a = 1/0
    ...
    def eliminarRutasSobrantes(self, rutas, nroVehiculos, capacidad):
        ...
        print("En eliminar rutas sobrantes...")
        ...
        #print(rutas)
        print("Salida de eliminar rutas sobrantes...")
